...
function RouteComponent() {
  ...
  return (
    <div
      className={cn("flex items-center justify-center h-full flex-col gap-4")}
    >
      {game?.status === "ended" && (
      ...
      >
        {game &&
          ...
          game.boards
            .sort(
              (a, b) =>
                Number(b.ownerId === player?.id) -
                Number(a.ownerId === player?.id),
            )
            .map((board) => (
              <Board
                key={board.ownerId}
      ...
      <Menu>
        {game?.status === "ended" && (
          <Button onClick={onGameRestart}>Сыграть ещё раз</Button>
        )}
        {game?.status !== "ended" && (
          <Button onClick={onGameEnd}>Завершить игру</Button>
        )}
      </Menu>
    </div>
  );
