    ...
    >
      ...
      <div className='flex bg-zinc-900 relative gap-2 p-2 text-xl'>
        <div className='flex gap-2 opacity-100 z-10 absolute p-2 top-0 left-0 h-full'>
          {allColumns.map((key) => (
            <Column key={key} onClick={() => onColumnClick(key)} available={isColumnAvailable(key)}>
              <AnimatePresence>
                {board.columns[key].map((value, index) => (
                  <Dice key={index} value={value} index={index} combos={diceCombo(key, value)} animate />
                ))}
              </AnimatePresence>
            </Column>
          ))}
